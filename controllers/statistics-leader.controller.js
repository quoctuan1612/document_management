...
module.exports.index = (request, response) => {
    ...
    let backgroundColorPie = ['rgb(255, 99, 132)',
                              'rgb(54, 162, 235)',
                              'rgb(255, 205, 86)'];
    let check_assignment = db.get("assignment").find({department_name: request.query.department}).value();
    let assignment = (db.get("assignment").value()).filter((value, index) => {
        return value.department_name === request.query.department;
    });
    ...
    response.render("statistics-leader", {
